...
public class Game implements Screen {
    ...
    public void draw(Graphics2D graphics2D) {
        ...
        if (updateCount < 90) {
            ...
            graphics2D.drawString(Main.gameState.getCurrentLevel().getLevelName(), text_start_point, 350);
        } else {

            graphics2D.setColor(WALL_COLOR);
            for (Wall wall : Main.gameState.getCurrentLevel().getWalls()) {
                graphics2D.fill(wall.getPolygon());
            }

            graphics2D.setColor(CHAR_COLOR);
            graphics2D.fill(Main.gameState.getCurrentLevel().getCharacter());

            graphics2D.setColor(FINNISH_COLOR);
            graphics2D.fill(Main.gameState.getCurrentLevel().getGoal());

            if (Main.gameState.getCurrentLevel().isWallTouched()) {
                graphics2D.setColor(FAILED_BACKGROUND_COLOR);
                graphics2D.fillRect(0, 0, panel.getWidth(), panel.getHeight());
                graphics2D.setColor(LEVEL_FAILED_FOREGROUND_COLOR);
                graphics2D.setFont(LEVEL_TITLE_FONT);
                String failedMessage = Main.gameState.getCurrentLevel().getLevelName() + " FAILED";
            ...
            if (Main.gameState.getCurrentLevel().isGoalReached()) {
                graphics2D.setColor(COMPLETED_BACKGROUND_COLOR);
                graphics2D.fillRect(0, 0, panel.getWidth(), panel.getHeight());
                graphics2D.setColor(LEVEL_COMPLETED_FOREGROUND_COLOR);
                graphics2D.setFont(LEVEL_TITLE_FONT);
                String endMessage = Main.gameState.getCurrentLevel().getLevelName() + " COMPLETE";
    ...
    public void update() {
        ...
        if (Main.gameState.getCurrentLevel().getCharacter().contains(Mouse.mousePos.getX(), Mouse.mousePos.getY()) && Mouse.mouseButtons[MouseEvent.BUTTON1]) {
            levelStartTimestamp = System.currentTimeMillis();
            Main.gameState.getCurrentLevel().setActive(true);
        }

        if (Main.gameState.getCurrentLevel().isActive()) {
        ...
        if (Main.gameState.getCurrentLevel().isActive()) {
            Main.gameState.getCurrentLevel().getCharacter().x = (int) Mouse.mousePos.getX() - 20;
            Main.gameState.getCurrentLevel().getCharacter().y = (int) Mouse.mousePos.getY() - 20;
        }

        Main.gameState.getCurrentLevel().calcWallTouch();
        Main.gameState.getCurrentLevel().calcGoalReached();
        ...
        if (Keyboard.keys[KeyEvent.VK_SPACE]) {
            ...
            if (Main.gameState.getCurrentLevel().isWallTouched()) {
                Main.gameState.reloadLevel();
                updateCount = 0;
            } else if (Main.gameState.getCurrentLevel().isGoalReached()) {
                Main.gameState.getNextLevel();
                updateCount = 0;
            }
        }
    }
